...
# from TorchGA3C.network.baseSNNNetwork import PseudoSpikeRect, erf, sigmoid
...
class SpikAttention(torch.nn.Module):
    ...
    def forward(
        ...
        if mask is not None:
            assert mask.shape == (bs, cur_seq), \
                f"expecting key_padding_mask shape of {(bs, cur_seq)}, but got {mask.shape}"

            mask = mask.view(bs, 1, 1, cur_seq). \
        ...
        attn_vec = attn_vec.permute(2, 0, 1, 3) / cur_seq
...
class SpikGatedTransformerLayer(torch.nn.Module):
    ...
    def forward(
        ...
        a1 = self.dropout(self.attention(inputs, mask=mask))
...
class SpikGTr(nn.Module):
    ...
    def forward(self, x: torch.Tensor, batch_first: bool = False, attn_mask: torch.Tensor = None) -> Dict[
        str, torch.Tensor]:
        ...
        for i in range(self.layer_num):
            layer = self.layers[i]
            out = layer(out, mask=attn_mask)  # cur_seq x bs x embedding_dim
...
if __name__ == "__main__":
    model = SpikGTr(input_dim=7, head_num=4, layer_num=1, embedding_dim=256)
    t = torch.rand(3, 1, 7)

    attn_mask = torch.randint(0, 2, (1, 3))

    out = model(t, attn_mask=attn_mask)
