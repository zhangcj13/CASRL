...
from layers.gtrxl import GTrXL

class GTrXLActor(BaseNet):
    def __init__(self, obs_size=4,oas_size=7, action_space=2,device=None,continuous_action_space=True):
        super(GTrXLActor, self).__init__(obs_size,oas_size, action_space,device)

        self.act_fea_transformer = GTrXL(input_dim=oas_size, head_num=8, layer_num=1,embedding_dim=256,gru_gating=True)
    ...
    def forward(self, x):
        ...
        trans_state = self.act_fea_transformer(other_seq, reset_mem=True)['logit']
...
class GTrXLCritic(BaseNet):
    def __init__(self, obs_size=4,oas_size=7, action_space=2,device=None):
        super(GTrXLCritic, self).__init__(obs_size,oas_size, action_space,device)

        self.crt_fea_transformer = GTrXL(input_dim=oas_size, head_num=8, layer_num=1,embedding_dim=512,gru_gating=True)
    ...
    def forward(self, x):
        ...
        # value
        trans_state = self.crt_fea_transformer(other_seq, reset_mem=True)['logit']
