...
class SpikGTrMeanActor(BaseNet):
    def __init__(self, obs_size=4,oas_size=7, action_space=2,device=None,continuous_action_space=True,timestep=3):
        super(SpikGTrMeanActor, self).__init__(obs_size,oas_size, action_space,device,timestep)
    ...
    def forward(self, x):
        ...
        mean_vec = torch.from_numpy(np.array([1 / (n ) for n in num_other_agents], dtype=np.float32)).to(other_seq.device)
        ...
        for step in range(self.timesteps):
            ...
            tran_mean = tran_sum * mean_vec

            # a_tran = torch.mean(trans_state, 0)
            fc1_v, fc1_s = self.mem_update(self.act_fc1, tran_mean, fc1_v, fc1_s)
