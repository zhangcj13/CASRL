...
class SpikGMFActor(BaseNet):
    def __init__(self, obs_size=4,oas_size=7, action_space=2,device=None,continuous_action_space=True,timestep=3):
        super(SpikGMFActor, self).__init__(obs_size,oas_size, action_space,device,timestep)
        self.act_fea = nn.Linear(oas_size, 512)
    ...
    def forward(self, x):
        ...
        oas_v = oas_s = torch.zeros(N,B, 512, device=self.device)
        ...
        for step in range(self.timesteps):
            oas_v, oas_s  = self.mem_update(self.act_fea, other_seq, oas_v, oas_s)

            tran_mems = torch.zeros(N + 1, B, 512, device=self.device)
            for sl in range(N):
                cur_mem = self.act_fc1(oas_s[sl])
