...
class SpikTrANActor(BaseNet):
    def __init__(self, obs_size=4,oas_size=7, action_space=2,device=None,continuous_action_space=True,timestep=3):
        super(SpikTrANActor, self).__init__(obs_size,oas_size, action_space,device,timestep)

        self.act_fea_transformer = SpikGTrAN(input_dim=oas_size, head_num=8, layer_num=1,embedding_dim=512,gru_gating=False,device=self.device)
