...
class GTrRNNActor(BaseNet):
    ...
    def forward(self, x):
        ...
        aout, _ = self.act_fea_lstm(trans_state, (ah, ac))
...
class GTrRNNCritic(BaseNet):
    def __init__(self, obs_size=4,oas_size=7, action_space=2,device=None):
        super(GTrRNNCritic, self).__init__(obs_size,oas_size, action_space,device)

        self.crt_fea_transformer = GTr(input_dim=oas_size, head_num=8, layer_num=1,embedding_dim=256,gru_gating=True)
        self.crt_fea_lstm = nn.LSTM(256, 256, 1, batch_first=False)
    ...
    def forward(self, x):
        ...
        # value
        trans_state = self.crt_fea_transformer(other_seq, attn_mask=key_padding_mask)['logit']

        vh = torch.zeros(1, B, 256).to(self.device)
        vc = torch.zeros(1, B, 256).to(self.device)

        vout, _ = self.crt_fea_lstm(trans_state, (vh, vc))
