...
def colors(x, name="tab20"):
    color=cm.get_cmap(plt.get_cmap(name))(x)
    # print(color)
    return color
...
algs = [
    ...
    {
        'name': 'SpikLSTM_DPPO_DC',
        'color': colors(5),
        'marker': markers(6),
    },
    {
        'name': 'SpikGTr_DPPO_DC',
        'color': colors(6),
        'marker': markers(7),
    },
    {
...
def plot(pdf_name='', num_agents_to_test=[],dir='./eval_output/stats'):
    df = pd.DataFrame()
    ...
    for num_agents in num_agents_to_test:
        for policy in algs:
            ...
            for seed in seeds:
                ...
                file_dir = os.path.dirname(os.path.realpath(__file__)) + '{}/{}_agents/'.format(dir,num_agents)
                log_filename = file_dir + '/stats_{}.p'.format(pol)
                try:
    ...
    df_failures = pd.pivot_table(df, index=['num_agents', 'policy_id'],aggfunc=aggfunc,dropna=False).reset_index(level=[0,1])

    # aggfunc2 = {}
    # for stat in stats:
    #     aggfunc2[stat['name']] = ['std']
    # df_std = pd.pivot_table(df, index=['num_agents', 'policy_id'], aggfunc=aggfunc2).reset_index(level=[0, 1])
    ...
    # fig.legend(handles, labels, loc='lower center', borderpad=0.1, labelspacing=0.1)

    plt.tight_layout()
    # fig.savefig('./test_time_bars_n_{}.pdf'.format(pdf_name)) # if latex exist...
    plt.show()
...
if __name__ == '__main__':
    # plot(pdf_name='_2_3_4', num_agents_to_test = [2,3,4],dir='/eval_output/stats_cadrl')
    plot(pdf_name='_6_8_10', num_agents_to_test=[6,8,10], dir='/eval_output/stats_cadrl')
