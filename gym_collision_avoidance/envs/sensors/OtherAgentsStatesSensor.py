...
class OtherAgentsStatesSensor(Sensor):
    ...
    def sense(self, agents, agent_index, top_down_map=None):
        ...
        other_agents_states = np.zeros((Config.MAX_NUM_OTHER_AGENTS_OBSERVED, other_state_length))
        if Config.OTHER_AGENTS_INIT_STATE is not None:
            other_agents_states = np.repeat(Config.OTHER_AGENTS_INIT_STATE, Config.MAX_NUM_OTHER_AGENTS_OBSERVED, axis=0)
