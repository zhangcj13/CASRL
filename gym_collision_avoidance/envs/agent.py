...
class Agent(object):
    ...
    def reset(self, px=None, py=None, gx=None, gy=None, pref_speed=None, radius=None, heading=None):
        ...
        other_state_length = 7
        self.ran_out_of_energy = False
        self.was_out_of_energy_already = False
        if Config.USE_ENERGY:
            self.energy = Config.RUNNING_ENERGY
            other_state_length = 8
        ...
        self.other_agent_states = np.zeros((other_state_length,))
    ...
    def take_action(self, action, dt):
        ...
        if (self.is_at_goal and Config.AGENT_STOP_IF_DONE['is_at_goal']) or (self.ran_out_of_time and Config.AGENT_STOP_IF_DONE['ran_out_of_time']) \
                or (self.in_collision and Config.AGENT_STOP_IF_DONE['in_collision']) or (self.ran_out_of_energy and Config.AGENT_STOP_IF_DONE['ran_out_of_energy']) :
            if self.is_at_goal:
                self.was_at_goal_already = True
            if self.in_collision:
                self.was_in_collision_already = True

            if self.ran_out_of_energy:
                self.was_out_of_energy_already = True
        ...
        if Config.USE_ENERGY:
            pre_pos=copy.deepcopy(self.pos_global_frame)
        ...
        if Config.USE_ENERGY:
            cur_pos=self.pos_global_frame
            move_direction = cur_pos - pre_pos
            energy_cost = math.sqrt(move_direction[0] ** 2 + move_direction[1] ** 2)
            self.energy -= energy_cost

            if self.energy <= 0.0:
                self.ran_out_of_energy = True
