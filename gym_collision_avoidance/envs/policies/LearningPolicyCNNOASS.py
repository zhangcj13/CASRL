...
try:
    ...
    from gym_collision_avoidance.envs.policies.DRL_Long.model.net import SOASS_CNNPolicy
except:
    print("Torch not installed...")

LASER_HIST = 3

class LearningPolicyCNNOASS(LearningPolicy):
    def __init__(self):
        # LearningPolicy.__init__(self, str="CNN_OASS")
    ...
    def initialize_network(self, **kwargs):
        ...
        policy = SOASS_CNNPolicy(action_space=2)
    ...
    def external_action_to_action(self, agent, external_action):
        ...
        [vx, vw] = external_action
        speed = agent.pref_speed * vx

        if Config.USING_MAX_HEADCHANGE:
            heading_change = vw * agent.max_heading_change
        else:
            heading_change = vw * Config.DT
