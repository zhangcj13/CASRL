...
# formation position
def generate_formation_case(num_agents, side_length, speed_bnds, radius_bnds):
	test_case = np.zeros((num_agents, 6))

	MIN_TRAVEL_DISTANCE = side_length / 1.0
	MAX_AGENT_DISTANCE_RATIO = 2.0

	agent_min_dis = radius_bnds[1] * 2.8
	agent_max_dis = agent_min_dis * MAX_AGENT_DISTANCE_RATIO

	rand_r = side_length * 1.2
	start = rand_r * 2 * np.random.rand(2, ) - rand_r
	end = rand_r * 2 * np.random.rand(2, ) - rand_r

	while np.linalg.norm(start - end) < MIN_TRAVEL_DISTANCE:
		end = side_length * 2 * np.random.rand(2, ) - side_length

	is_line = np.random.rand() > 0.5 #0.5

	adis = np.random.rand() * (agent_max_dis - agent_min_dis) + agent_min_dis
	if is_line:

		vector=end-start
		lengtn = np.sqrt(sum(x ** 2 for x in vector))
		unit_vector = vector/lengtn
		vertical_vector = np.array([unit_vector[1],-unit_vector[0]])*adis
		bound=num_agents//2
		for i in range(num_agents):
			test_case[i, 5] = (radius_bnds[1] - radius_bnds[0]) \
							  * np.random.rand() + radius_bnds[0]
			...
			test_case[i, 0:2] = start+(i-bound)*vertical_vector
			test_case[i, 2:4] = end+(i-bound)*vertical_vector
	else:
		for i in range(num_agents):
			test_case[i, 5] = (radius_bnds[1] - radius_bnds[0]) \
							  * np.random.rand() + radius_bnds[0]
			...
			vector = 1.1 * agent_min_dis * np.random.uniform(-1,1,2 )
			agent_pos = start + vector
			if i ==0:
				test_case[i, 0:2] = agent_pos
				test_case[i, 2:4] = end + vector
			else:
				min_dis=1e6
				for j in range(i):
					dis = np.linalg.norm(agent_pos - test_case[j, 0:2])
					if dis<min_dis:
						min_dis=dis

				while min_dis<agent_min_dis:
					vector = 1.1 * agent_min_dis * np.random.uniform(-1, 1, 2)
					agent_pos = start + vector
					min_dis = 1e6
					for j in range(i):
...
def generate_formation_case_LaF(num_agents, side_length, speed_bnds, radius_bnds):
	test_case = np.zeros((num_agents, 6))
...
if __name__ == '__main__':
	...
	for i in range(num_test_cases):
		...
		# test_case = generate_circle_case(num_agents, side_length, speed_bnds, radius_bnds)
