...
class CollisionAvoidanceEnv(gym.Env):
    ...
    def __init__(self):
        ...
        self.ampl2goal=Config.AMPL_GETING_CLOSE2GOAL
    ...
    def step(self, actions, dt=None):
        ...
        """
        if 'ampl2goal' in actions.keys():
            self.ampl2goal = actions['ampl2goal']
            actions.pop('ampl2goal', None)
        ...
        if game_over and Config.EVALUATE_MODE:
            return next_observations, rewards, game_over, {'agents':copy.deepcopy(self.agents)}

        which_agents_done_dict = {}
        which_agents_learning_dict = {}
        which_agents_status_dict = {}
        for i, agent in enumerate(self.agents):
            ...
            agent_status = 'Runing'
            if agent.is_at_goal:
                agent_status = 'Reach Goal'
            elif agent.in_collision:
                agent_status = 'Crashed'
            elif agent.ran_out_of_time:
                agent_status = 'Time out'

            which_agents_status_dict[agent.id]=agent_status
        ...
        return next_observations, rewards, game_over, \
            {
                ...
                'which_agents_status_dict':which_agents_status_dict,
            }
    ...
    def _compute_rewards(self):
        ...
        if Config.COLLABORATIVE:
            return self._compute_rewards_c()
        ...
        for i, agent in enumerate(self.agents):
            ...
            # if self.ampl2goal<0.01:
            #     print('Config.ampl2goal: ',self.ampl2goal)
            reward_g[i] = (self.dis2goal_pre[i] - self.dis2goal_cur[i])*self.ampl2goal #1.0 0.12
            rewards[i] += reward_g[i]
            ...
            if agent.is_at_goal:
                if agent.was_at_goal_already is False:
                    ...
                    if Config.USE_ENERGY:
                        reward_en = agent.energy/Config.RUNNING_ENERGY
                        rewards[i]+= np.max((reward_en,-0.6))
                else:
                    rewards[i] *= 4
            else:
                ...
                if agent.was_in_collision_already is False:
                    if collision_with_agent[i]:
                        ...
                        if Config.USE_ENERGY:
                            if dist_btwn_nearest_agent[i]<0.4 and dist_btwn_nearest_agent[i]> Config.GETTING_CLOSE_RANGE:
                                rewards[i]+=0.05
                                agent.energy+=0.1
                        ...
                        if dist_btwn_nearest_agent[i] <= Config.GETTING_CLOSE_RANGE:
                            if not Config.NEW_REWARD_TYPE:
                                ...
                                rewards[i] += (dist_btwn_nearest_agent[i]*10 - 1.0)*0.5 # 4gents:1.0, 10agents:0.5
        ...
        if Config.COLLABORATIVE:
            r = np.sum(rewards)
            rewards = r * np.ones(len(self.agents))
    ...
    def _check_which_agents_done(self):
        ...
        # ran_out_of_energy_condition = np.array([a.ran_out_of_energy for a in self.agents])

        at_goal_condition = np.array([a.is_at_goal if Config.AGENT_STOP_IF_DONE['is_at_goal'] else False for a in self.agents])
        ran_out_of_time_condition = np.array([a.ran_out_of_time if Config.AGENT_STOP_IF_DONE['ran_out_of_time'] else False for a in self.agents])
        in_collision_condition = np.array([a.in_collision if Config.AGENT_STOP_IF_DONE['in_collision'] else False for a in self.agents])
        ran_out_of_energy_condition = np.array([a.ran_out_of_energy if Config.AGENT_STOP_IF_DONE['ran_out_of_energy'] else False for a in self.agents])

        if Config.USE_ENERGY:
            which_agents_done = np.logical_or.reduce((at_goal_condition, ran_out_of_time_condition, in_collision_condition,ran_out_of_energy_condition))
        else:
    ...
    def _compute_rewards_c(self):
        ...
        reward_g = np.zeros(len(self.agents))
        reward_bwt = np.zeros(len(self.agents))

        for i, agent in enumerate(self.agents):
            reward_g[i] = -agent.dist_to_goal * self.ampl2goal  # 1.0 0.12
            rewards[i] += reward_g[i]

            max_dis =-1
            for j, o_agent in enumerate(self.agents):
                if i==j:
                    continue
                dist_btwn = l2norm(agent.pos_global_frame, o_agent.pos_global_frame)
                combined_radius = agent.radius + o_agent.radius
                dist_ = dist_btwn - combined_radius
                max_dis = dist_ if dist_btwn>max_dis else max_dis
            reward_bwt[i]= -(max_dis-0.2)*self.ampl2goal*0.2

            rewards[i] += reward_bwt[i]
