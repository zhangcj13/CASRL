...
from gym_collision_avoidance.envs.policies.LearningPolicyPPOs import LearningPolicyPPOs
...
policy_dict = {
    ...
    'CNNOAS': LearningPolicyCNNOASS,
    'learning_ppos':LearningPolicyPPOs,
}
...
def get_testcase_random(num_agents=None, side_length=4, speed_bnds=[0.5, 2.0], radius_bnds=[0.2, 0.8], policies='learning', policy_distr=None, agents_dynamics='unicycle', agents_sensors=['other_agents_states'], policy_to_ensure=None, prev_agents=None):
    if num_agents is None:
        if not Config.FIXED_AGENTS_NUM:
...
def get_testcase_queue(num_agents=None, side_length=4, speed_bnds=[0.5, 2.0], radius_bnds=[0.2, 0.8], policies='learning', policy_distr=None, agents_dynamics='unicycle', agents_sensors=['other_agents_states'], policy_to_ensure=None, prev_agents=None):
    # x1 = np.random.uniform(0.3, 5)
    # x2 = np.random.uniform(-5, -0.3)
    #
    # agents = [
    #     Agent( x1, -5.,  1.,  5., 0.3, 1.0, np.pi/2, policy_dict[policy_to_ensure], UnicycleDynamics, [OtherAgentsStatesSensor], 0),
    #     Agent( x2, -5., -1.,  5., 0.3, 1.0, np.pi/2, policy_dict[policy_to_ensure], UnicycleDynamics, [OtherAgentsStatesSensor], 1),
    #     # Agent(-2., -3., -2.,  3., 0.3, 1.0, np.pi/2, policy_dict[policy_to_ensure], UnicycleDynamics, [OtherAgentsStatesSensor], 2),
    # ]

    agents = [
        Agent( 2., -5.,  2., 5., 0.3, 1.0, np.pi / 2, policy_dict[policy_to_ensure], UnicycleDynamics, [OtherAgentsStatesSensor], 0),
        Agent( 1., -5.,  1., 5., 0.3, 1.0, np.pi / 2, policy_dict[policy_to_ensure], UnicycleDynamics, [OtherAgentsStatesSensor], 1),
        Agent( 0., -5.,  0., 5., 0.3, 1.0, np.pi / 2, policy_dict[policy_to_ensure], UnicycleDynamics, [OtherAgentsStatesSensor], 2),
        Agent(-1., -5., -1., 5., 0.3, 1.0, np.pi / 2, policy_dict[policy_to_ensure], UnicycleDynamics, [OtherAgentsStatesSensor], 3),
        Agent(-2., -5., -2., 5., 0.3, 1.0, np.pi / 2, policy_dict[policy_to_ensure], UnicycleDynamics, [OtherAgentsStatesSensor], 4),
    ]
...
def cadrl_test_case_to_agents(test_case, policies='GA3C_CADRL', policy_distr=None,
    ...
    prev_agents=None, force_agent_face_goal=False):
    ###############################
    ...
    if type(policies) == str:
        ...
        if Config.TRAIN_SINGLE_AGENT:
            agent_policy_list = np.random.choice(policies,
                                                 num_agents,
                                                 p=policy_distr)
            agent_policy_list[0] = policy_to_ensure
            # print(agent_policy_list)
        else:
    ...
    for i, agent in enumerate(test_case):
        ...
        pref_speed = agent[4]
        radius = agent[5]
        if Config.EVALUATE_MODE or force_agent_face_goal:
...
import pandas as pd
if Config.EVALUATE_MODE:
    # file = '../gym_collision_avoidance/envs/test_cases/{}_agents_{}_cases.p'.format(Config.MAX_NUM_AGENTS_IN_ENVIRONMENT,Config.NUM_TEST_CASES)
    file = './gym_collision_avoidance/envs/test_cases/{}_agents_{}_cases.p'.format(Config.MAX_NUM_AGENTS_IN_ENVIRONMENT,Config.NUM_TEST_CASES)
    
    agents_tcs_500 = pd.read_pickle(file)
    agents_tcs_id =0

def get_500_test_case(num_agents=None, side_length=4, speed_bnds=[0.5, 2.0], radius_bnds=[0.2, 0.8], policies='learning', policy_distr=None, agents_dynamics='unicycle', agents_sensors=['other_agents_states'], policy_to_ensure=None, prev_agents=None):
    global agents_tcs_id

    if agents_tcs_id>=500:
        if type(side_length) is list:
            # side_length lists (range of num_agents, range of side_lengths) dicts
            # to enable larger worlds for larger nums of agents (to somewhat maintain density)
            for comp in side_length:
        ...
        cadrl_test_case = agents_tcs_500[agents_tcs_id]
        agents_tcs_id += 1
...
# Formation
def get_formation_random(num_agents=None, side_length=4, speed_bnds=[0.5, 2.0], radius_bnds=[0.2, 0.8], policies='learning', policy_distr=None, agents_dynamics='unicycle', agents_sensors=['other_agents_states'], policy_to_ensure=None, prev_agents=None):
    if num_agents is None:
        if not Config.FIXED_AGENTS_NUM:
            num_agents = np.random.randint(2, Config.MAX_NUM_AGENTS_IN_ENVIRONMENT+1)
        else:
    ...
    # cadrl_test_case = tc.generate_rand_test_case_multi(num_agents, side_length, speed_bnds, radius_bnds)
    if Config.COLLABORATIVE==True:
        if Config.COLLABORATIVE_TYPE is 'LaF':
            test_case = tc.generate_formation_case(num_agents, side_length, speed_bnds, radius_bnds)
        else:
            test_case = tc.generate_formation_case(num_agents, side_length, speed_bnds, radius_bnds)
    else:
        test_case = tc.generate_formation_case(num_agents, side_length, speed_bnds, radius_bnds)
    ...
    agents = cadrl_test_case_to_agents(test_case,
        ...
        policy_to_ensure=policy_to_ensure,
        prev_agents=prev_agents,
        force_agent_face_goal=True,
        )
    return agents
# leader-and-follower formation
def get_formation_random_LaF(num_agents=None, side_length=4, speed_bnds=[0.5, 2.0], radius_bnds=[0.2, 0.8], policies='learning', policy_distr=None, agents_dynamics='unicycle', agents_sensors=['other_agents_states'], policy_to_ensure=None, prev_agents=None):
    if num_agents is None:
        if not Config.FIXED_AGENTS_NUM:
            num_agents = np.random.randint(2, Config.MAX_NUM_AGENTS_IN_ENVIRONMENT+1)
        else:
    ...
    cadrl_test_case = tc.generate_formation_case(num_agents, side_length, speed_bnds, radius_bnds)
